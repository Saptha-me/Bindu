...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('message_history', postgresql.JSONB(astext_type=sa.Text()), nullable=True, server_default='[]'),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('NOW()')),
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('NOW()')),
    ...
    op.create_index('idx_tasks_updated_at', 'tasks', ['updated_at'], postgresql_ops={'updated_at': 'DESC'})
    ...
    # Contexts indexes
    op.create_index('idx_contexts_created_at', 'contexts', ['created_at'], postgresql_ops={'created_at': 'DESC'})
    op.create_index('idx_contexts_updated_at', 'contexts', ['updated_at'], postgresql_ops={'updated_at': 'DESC'})
    op.create_index('idx_contexts_data_gin', 'contexts', ['context_data'], postgresql_using='gin')
    op.create_index('idx_contexts_history_gin', 'contexts', ['message_history'], postgresql_using='gin')
    
    # Task feedback indexes
    op.create_index('idx_task_feedback_task_id', 'task_feedback', ['task_id'])
    op.create_index('idx_task_feedback_created_at', 'task_feedback', ['created_at'], postgresql_ops={'created_at': 'DESC'})
