...
"""The Language of Agents: A2A Protocol Types.
...
where A2A (Agent-to-Agent) comes alive.
...
# -----------------------------------------------------------------------------
# Shared Pydantic Config for A2A Types
# -----------------------------------------------------------------------------
#
# Every TypedDict in this module maps snake_case Python attribute names
# (``message_id``, ``context_id``, ``task_id``) to camelCase keys on the wire
# (``messageId``, ``contextId``, ``taskId``) via ``alias_generator=to_camel``.
# The server emits with ``by_alias=True`` so responses are always camelCase —
# this matches A2A spec v0.3.0 and every external client (TypeScript SDK,
# Postman collection, the deployed OpenAPI specs).
#
# ``populate_by_name=True`` makes the server *also accept* the Python
# attribute name on input. Without it, pydantic treats the alias as the only
# valid input key and silently drops snake_case keys a Python caller wrote,
# then errors with "messageId: Field required" — a DX landmine where the
# rejected keys are the ones your own IDE autocompletes. With it, either
# form works on input while output remains strictly camelCase.
#
# See bugs/2026-04-18-types-populate-by-name.md for the full rationale.
