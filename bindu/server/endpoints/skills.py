...
async def skill_detail_endpoint(
    ...
    if len(path_parts) < 4:
        code, message = extract_error_fields(NotFoundError)
...
async def skill_documentation_endpoint(
    ...
    try:
        ...
        if not documentation:
            ...
            return jsonrpc_error(
                code,
                f"No documentation available for skill: {skill_id}",
                status=404
            )
        ...
        logger.error(f"Error serving skill documentation to {client_ip}: {e}", exc_info=True)
        code, message = extract_error_fields(InternalError)
        return jsonrpc_error(code, message, str(e), status=500)
