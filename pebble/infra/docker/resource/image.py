...
from pebble.infra.docker.api_client import DockerApiClient
from pebble.infra.docker.resource.base import DockerResource
...
class DockerImage(DockerResource):
    ...
    def build_image(self, docker_client: DockerApiClient) -> Optional[Any]:
        ...
        logger.info(f"Building image: {self.get_image_str()}")
        nocache = self.skip_docker_cache or self.force
        pull = self.pull or self.force
        if self.path is not None:
            logger.info(f"\t  path: {self.path}")
        ...
        try:
            ...
            if self.push_image:
                logger.info(f"Pushing {self.get_image_str()}")
                with Live(transient=True, console=console) as live_log:
                    push_status = {}
                    ...
                    for push_output in _api_client.images.push(
                        ...
                        if push_output.get("aux", {}).get("Size", 0) > 0:
                            logger.info(f"Push complete: {push_output.get('aux', {})}")
    ...
    def create(self, docker_client: DockerApiClient) -> bool:
        ...
        if not self.force:
            ...
            if self.use_cache and self.is_active(docker_client=docker_client):
                logger.info(f"{self.get_resource_type()}: {self.get_resource_name()} already exists")
                return True

        resource_created = self._create(docker_client=docker_client)
        if resource_created:
            logger.info(f"{self.get_resource_type()}: {self.get_resource_name()} created")
            return True
