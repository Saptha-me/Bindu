...
class HibiscusClient:
    ...
    async def request_api_challenge(self) -> Dict[str, Any]:
        ...
        """
        payload = {"api_key": str(self.pat_token.get_secret_value()), "email": self.email}

        try:
            response = await make_api_request(url=self.auth_challenge_endpoint, method="POST", payload=payload)
    ...
    async def get_api_token(self, challenge: str) -> str:
        ...
        Returns:
            JWT access token
        """
        payload = {"api_key": str(self.pat_token.get_secret_value()), "email": self.email, "challenge": challenge}

        try:
            response = await make_api_request(url=self.auth_token_endpoint, method="POST", payload=payload)
    ...
    async def register_agent(self, agent_manifest: AgentManifest, **kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            capabilities = {"push_notifications": None, "state_transition_history": None, "streaming": None}
            ...
            if (
                agent_manifest
                and agent_manifest.identity
                and hasattr(agent_manifest.identity, "did_document")
                and agent_manifest.identity.did_document
            ):
            ...
            try:
                ...
                response = await make_api_request(
                    url=self.agents_endpoint, method="POST", payload=payload, headers=headers
                )
