...
def pebblify(
    ...
    def decorator(agent_function: Callable) -> AgentManifest:
        ...
        did_extension = DIDAgentExtension(recerate_keys=recreate_keys, key_dir=Path(os.path.join(caller_dir, PKI_DIR)))
        ...
        if capabilities and capabilities.get("extensions", None) is not None:
            capabilities["extensions"].append(did_extension.agent_extension)
        elif capabilities and capabilities.get("extensions", None) is None:
