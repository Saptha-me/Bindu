...
class CertificateHandler:
    ...
    def __init__(self, did_manager: DIDManager, certificate_manager: CertificateManager):
    ...
    async def handle_certificate_status(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            response = {
                ...
                "token_status": {"valid": token_valid, "expires_in_seconds": token_expires_in},
            }
            ...
            return web.json_response({"error": f"Failed to get certificate status: {str(e)}"}, status=500)
    ...
    async def handle_verify_connection(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if not peer_did:
                return web.json_response({"error": "Missing DID in request"}, status=400)

            # Return connection verification result
            return web.json_response({"verified": True, "did": peer_did, "timestamp": time.time()})

        except Exception as e:
            logger.error(f"Error verifying connection: {str(e)}")
            return web.json_response({"error": f"Connection verification failed: {str(e)}"}, status=500)
