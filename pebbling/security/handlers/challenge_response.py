...
class ChallengeResponseHandler:
    ...
    def _clean_expired_challenges(self) -> None:
        ...
        expired_challenges = [
            challenge_id for challenge_id, challenge in self.challenges.items() if challenge["expires"] < current_time
        ]
    ...
    async def handle_challenge(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            # Store the challenge
            self.challenges[challenge_id] = {"challenge": challenge_text, "expires": expires, "peer_did": peer_did}

            # Prepare response
            response_data = {"challenge_id": challenge_id, "challenge": challenge_text, "expires": expires}

            return web.json_response(response_data)

        except Exception as e:
            logger.error(f"Error generating challenge: {str(e)}")
            return web.json_response({"error": f"Challenge generation failed: {str(e)}"}, status=500)
    ...
    async def handle_challenge_response(self, request: web.Request) -> web.Response:
        ...
        try:
            ...
            if challenge_id not in self.challenges:
                return web.json_response({"error": "Challenge not found or expired"}, status=400)
            ...
            if challenge["expires"] < time.time():
                del self.challenges[challenge_id]
                return web.json_response({"error": "Challenge expired"}, status=401)

            # Check if challenge was issued for this DID
            if challenge["peer_did"] != peer_did:
                return web.json_response({"error": "Challenge was not issued for this DID"}, status=401)
            ...
            try:
                ...
                if not peer_did_document:
                    return web.json_response(
                        {"error": "Peer DID document not available. Please exchange DIDs first."}, status=400
                    )
                ...
                if not verification_method:
                    ...
                    return web.json_response({"error": "No verification method found in DID document"}, status=400)
                ...
                is_valid_signature = await self.did_manager.verify_message(
                    challenge_message, signature, verification_method
                )
            ...
            current_time = time.time()
            self.verified_connections[peer_did] = {"verified": True, "last_verified": current_time}

            # Clean up the challenge
            del self.challenges[challenge_id]

            return web.json_response({"authenticated": True, "did": peer_did, "timestamp": current_time})

        except Exception as e:
            logger.error(f"Error handling challenge response: {str(e)}")
            return web.json_response({"error": f"Challenge response failed: {str(e)}"}, status=500)
