...
class VerificationMiddleware:
    ...
    def __init__(self, challenge_response_handler, certificate_manager=None):
    ...
    async def verify_security(self, request: web.Request, handler: Callable) -> web.Response:
        ...
        if not self.challenge_response_handler.is_connection_verified(did):
            ...
            return web.json_response({"error": "Connection not verified. Please authenticate first."}, status=401)
        ...
        if self.certificate_manager:
            ...
            try:
                ...
                if not transport:
                    ...
                    return web.json_response({"error": "No transport in request"}, status=500)

                ssl_object = transport.get_extra_info("ssl_object")
                if not ssl_object:
                    logger.error("No SSL object in transport")
                    return web.json_response({"error": "No SSL connection"}, status=401)

                # Get certificate in DER format
                peer_cert_bin = ssl_object.getpeercert(binary_form=True)
                if not peer_cert_bin:
                    logger.error("No peer certificate")
                    return web.json_response({"error": "No client certificate provided"}, status=401)
                ...
                if not is_valid:
                    ...
                    return web.json_response({"error": "Invalid certificate"}, status=401)

            except Exception as e:
                logger.error(f"Error validating certificate: {str(e)}")
                return web.json_response({"error": f"Certificate validation failed: {str(e)}"}, status=401)
