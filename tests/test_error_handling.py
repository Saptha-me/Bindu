...
class TestErrorHandling:
    ...
    def test_health_check_error(self, mock_error_response, client, protocol_handler):
        ...
        mock_error = ErrorResponse(
            status_code=500,
            status="error",
            message="Health check failed: Agent error"
        )
        mock_error_response.return_value = JSONResponse(
            status_code=500,
            content=mock_error.model_dump()
        )
    ...
    def test_act_error(self, mock_error_response, client, protocol_handler):
        ...
        mock_error = ErrorResponse(
            status_code=500,
            status="error",
            message="Agent execution failed: Action error"
        )
        mock_error_response.return_value = JSONResponse(
            status_code=500,
            content=mock_error.model_dump()
    ...
    def test_listen_error(self, mock_error_response, client, protocol_handler, audio_request_json):
        ...
        mock_error = ErrorResponse(
            status_code=500,
            status="error",
            message="Audio processing failed: Listen error"
        )
        mock_error_response.return_value = JSONResponse(
            status_code=500,
            content=mock_error.model_dump()
    ...
    def test_view_error(self, mock_error_response, client, protocol_handler, image_request_json):
        ...
        mock_error = ErrorResponse(
            status_code=500,
            status="error",
            message="Media processing failed: View error"
        )
        mock_error_response.return_value = JSONResponse(
            status_code=500,
            content=mock_error.model_dump()
